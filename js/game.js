...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		let gain = tmp[layer].baseAmount.div(tmp[layer].requires).div(tmp[layer].gainMult).max(1).log(tmp[layer].base).times(tmp[layer].gainExp).pow(MetaNum.pow(tmp[layer].exponent, -1))
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (tmp[layer].gainMult.lte(0)) return new MetaNum(Infinity)
	if (tmp[layer].gainExp.lte(0)) return new MetaNum(Infinity)
	...
	if (type=="static") 
	{
		...
		let extraCost = MetaNum.pow(tmp[layer].base, amt.pow(tmp[layer].exponent).div(tmp[layer].gainExp)).times(tmp[layer].gainMult)
